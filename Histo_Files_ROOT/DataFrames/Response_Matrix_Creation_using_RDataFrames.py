...
def parse_args():
    ...
    parser.add_argument('-cr', '-crho', '--cut_rho0',
                        type=str,
                        default="",
                        choices=['', "Remove_rho", "Require_rho", "Exclusive_rho", "Non_Exclusive_rho"],
                        help=f"Unique cut options which apply only to the rho0 particles. Option Meanings are:\n\t(*) '' --> No special cut,\n\t(*) 'Remove_rho' --> 'Par_PID_pip!=113' (removes ALL rho0's from the events)\n\t(*) 'Require_rho' --> 'Par_PID_pip==113' (requires events to have a rho0 parent)\n\t(*) 'Exclusive_rho' --> 'exclusive_rho==1' (requires the strictest method of finding the diffractive rho0 events)\n\t(*) 'Non_Exclusive_rho' --> 'exclusive_rho==0' (removes all rho0s that are known to come from exclusive events)\n\t\t{color.Error}Warning: in experimental data, 'Non_Exclusive_rho' removes events based on particles detected, so it will not be known whether this is actually removing the rho0-based events.{color.END}\n")
    ...
    parser.add_argument('-old3D', '--old_3D_unfold',
                        action='store_true',
                        help=f"Uses the legacy sparse 3D MultiDim bins with a fixed 915-bin axis {color.RED}(default is dense 3D packing per Q2-y bin){color.END}.\n")
    ...
    parser.add_argument('-hpp_in', '--hpp_input_file',
                        ...
                        help="Pure-acceptance HPP (accw_* symbols) for Event_Weight_Acc.\n")
    parser.add_argument('-hpp_sw', '--hpp_input_file_spline',
                        type=str,
                        default=None,
                        help="Combined Acc+physics HPP (accw_sw_* symbols) for Event_Weight_AccSpline/AccJSON. Falls back to --hpp_input_file if unset.\n")
...
def Create_z1_plus_z2(args, df, df_name="rdf"):
    for col_type in ["", "_gen", "_smeared"]:
        if((col_type not in [""]) and ("mdf" not in str(df_name))):
            continue
        if(not df.HasColumn(f"z1_plus_z2{col_type}")):
            Update_Email(args, update_message=f"\t{color.Error}WARNING: '{df_name}' is missing 'z1_plus_z2{col_type}'{color.END}", verbose_override=True, no_time=True)
            if(all(df.HasColumn(needed) for needed in [f"z{col_type}", f"z_pim{col_type}", "pim_present"])):
                df = df.Define(f"z1_plus_z2{col_type}", f"""
                double z1_plus_z2{col_type} = 0.0;
                if(pim_present != 0) {{ z1_plus_z2{col_type} = (z{col_type} + z_pim{col_type}); }} 
                return z1_plus_z2{col_type};
                """)
            else:
...
def Make_exclusive_rho_Flags(args, df, dfname, lundrho_files=""):
    ...
    if(getattr(args, "run_rho_weight", False) and (not df.HasColumn("exclusive_rho_weight")) and all(df.HasColumn(needed_rho) for needed_rho in ["exclusive_rho"])):
        weight_with_rho = 1.0 if(dfname == "rdf") else 0.0 if(lundrho_files in [""]) else 4.526912 if(lundrho_files in ["lundrho"]) else 4.624974 if(lundrho_files in ["lundvpk"]) else 1.0
        weight_wout_rho = 1.0 if(dfname == "rdf") else 1.0 if(lundrho_files in [""]) else 4.526912 if(lundrho_files in ["lundrho"]) else 4.624974 if(lundrho_files in ["lundvpk"]) else 1.0
        # Invalid (rho weights were already applied to the ROOT inputs used to derive these factors — do not use):
        # weight_with_rho = 1.0 if(dfname == "rdf") else 0.0 if(lundrho_files in [""]) else 1.084134 if(lundrho_files in ["lundrho"]) else 1.083707 if(lundrho_files in ["lundvpk"]) else 1.0
        # weight_wout_rho = 1.0 if(dfname == "rdf") else 1.0 if(lundrho_files in [""]) else 1.084134 if(lundrho_files in ["lundrho"]) else 1.083707 if(lundrho_files in ["lundvpk"]) else 1.0 
        df = df.Define("exclusive_rho_weight", f'''double exclusive_rho_weight = 1.0;
            if(exclusive_rho == 1){{ exclusive_rho_weight = {weight_with_rho}; }}
            if(exclusive_rho == 0){{ exclusive_rho_weight = {weight_wout_rho}; }}
...
if(__name__ == "__main__"):
    ...
    if(args.cut_rho0 not in [""]):
        Update_Email(args, update_message=f"{color.BBLUE}APPLYING SELECTED rho0 CUTS{color.END}", verbose_override=True)
        if(args.cut_rho0 in ["Remove_rho", "Require_rho"]):
            if("Par_PID_pip" in mdf_clasdis.GetColumnNames()):
                mdf_clasdis = mdf_clasdis.Filter("Par_PID_pip!=113" if(args.cut_rho0 in ["Remove_rho"]) else "Par_PID_pip==113")
            else:
                Update_Email(args, update_message=f"\n{color.RED}WARNING: rho0 cut '{args.cut_rho0}' failed to be applied to the mdf file(s).{color.END}\n", verbose_override=True)
            if("Par_PID_pip" in gdf_clasdis.GetColumnNames()):
                gdf_clasdis = gdf_clasdis.Filter("Par_PID_pip!=113" if(args.cut_rho0 in ["Remove_rho"]) else "Par_PID_pip==113")
            else:
                Update_Email(args, update_message=f"\n{color.RED}WARNING: rho0 cut '{args.cut_rho0}' failed to be applied to the gdf file(s).{color.END}\n", verbose_override=True)
        else:
            if("exclusive_rho" in rdf.GetColumnNames()):
                rdf = rdf.Filter("(exclusive_rho==1) && (pim_present==1) && (proton_present==1)" if(args.cut_rho0 not in ["Non_Exclusive_rho"]) else "exclusive_rho==0")
            else:
                Update_Email(args, update_message=f"\n{color.RED}WARNING: rho0 cut '{args.cut_rho0}' failed to be applied to the rdf file(s).{color.END}\n", verbose_override=True)
            if("exclusive_rho" in mdf_clasdis.GetColumnNames()):
                mdf_clasdis = mdf_clasdis.Filter("exclusive_rho==1" if(args.cut_rho0 not in ["Non_Exclusive_rho"]) else "exclusive_rho==0")
            else:
                Update_Email(args, update_message=f"\n{color.RED}WARNING: rho0 cut '{args.cut_rho0}' failed to be applied to the mdf file(s).{color.END}\n", verbose_override=True)
            if("exclusive_rho" in gdf_clasdis.GetColumnNames()):
                gdf_clasdis = gdf_clasdis.Filter("exclusive_rho==1" if(args.cut_rho0 not in ["Non_Exclusive_rho"]) else "exclusive_rho==0")
            else:
                Update_Email(args, update_message=f"\n{color.RED}WARNING: rho0 cut '{args.cut_rho0}' failed to be applied to the gdf file(s).{color.END}\n", verbose_override=True)
    if(args.Use_EvGen):
    ...
    if(args.make_root):
        ...
        # Dense 3D is default; -old3D restores sparse MultiDim + fixed 915-bin axes.
        use_dense_3d = ((not getattr(args, "old_3D_unfold", False)) and (not args.valerii_bins))
        use_dense_5d = (not (getattr(args, "old_5D_unfold", True) or args.valerii_bins))
        def _var_type_suffix(var_type=""):
            if(str(var_type) in ["smear", "smeared", "_smeared", "Smear", "Smeared", "_Smeared"]):
                return "_smeared"
            if(str(var_type) in ["GEN", "Gen", "gen", "_GEN", "_Gen", "_gen"]):
                return "_gen"
            return ""
        def ensure_z_pT_Bin_Y_bin(df_in, var_type=""):
            # Redefine z_pT_Bin_Y_bin from raw z/pT via ExtraAnalysisCodeValues.Find_z_pT_Bin (not Q2_Y_Bin). Y_bin top-row z_max for Q2-y 3/7/11/15/17 updated 08-04-2026 in ExtraAnalysisCodeValues.New_z_pT_and_MultiDim_Binning_Code, Binning_Dictionaries.Full_Bin_Definition_Array, and MyCommonAnalysisFunction_richcap.Bin_Definition_Array (+ skip_condition_z_pT_bins). Sparse Phi_h_Bin_Values MultiDim offsets are not renumbered by new skips; dense MultiDim Redefine below uses skip_condition_z_pT_bins.
            suf = _var_type_suffix(var_type)
            col  = f"z_pT_Bin_Y_bin{suf}"
            qcol = f"Q2_Y_Bin{suf}"
            zcol = f"z{suf}"
            pcol = f"pT{suf}"
            if(not (df_in.HasColumn(qcol) and df_in.HasColumn(zcol) and df_in.HasColumn(pcol))):
                return df_in
            code = Multi_Bin_Standard_Def_Function(Variable_Type=var_type, Dimension="2D", Use_Dense_Binning=False, args=args)
            if(df_in.HasColumn(col)):
                return df_in.Redefine(col, code)
            return df_in.Define(col, code)
        def ensure_defined_col(df_in, col_name, code, warn_label=None):
            if(not df_in.HasColumn(col_name)):
                if(warn_label is not None):
                    print(f"\t{color.Error}WARNING: {warn_label} is missing '{col_name}'{color.END}")
                return df_in.Define(col_name, code)
            return df_in.Redefine(col_name, code)
        def ensure_multidim_3d(df_in, col_name, var_type="", warn_label=None):
            code = Multi_Bin_Standard_Def_Function(Variable_Type=var_type, Dimension="3D", Use_Dense_Binning=use_dense_3d, args=args)
            if(not df_in.HasColumn(col_name)):
                if(warn_label is not None):
                    print(f"\t{color.Error}WARNING: {warn_label} is missing '{col_name}'{color.END}")
                return df_in.Define(col_name, code)
        ...
        check_frame = True
        if(check_frame):
            rdf = ensure_z_pT_Bin_Y_bin(rdf, "")
            rdf = ensure_multidim_3d(rdf, "MultiDim_z_pT_Bin_Y_bin_phi_t", "", "        'rdf'")
            rdf = ensure_defined_col(rdf, "MultiDim_Q2_y_z_pT_phi_h", Multi_Bin_Standard_Def_Function(Variable_Type="", Dimension="5D", Use_Dense_Binning=use_dense_5d, args=args), "        'rdf'")
            rdf = ensure_defined_col(rdf, "Q2_y_z_pT_4D_Bins", Q2_y_z_pT_4D_Bin_Def_Function_New(Variable_Type=""), "        'rdf'")
            ...
            mdf_clasdis = ensure_z_pT_Bin_Y_bin(mdf_clasdis, "")
            mdf_clasdis = ensure_multidim_3d(mdf_clasdis, "MultiDim_z_pT_Bin_Y_bin_phi_t", "", "'mdf_clasdis'")
            mdf_clasdis = ensure_defined_col(mdf_clasdis, "MultiDim_Q2_y_z_pT_phi_h", Multi_Bin_Standard_Def_Function(Variable_Type="", Dimension="5D", Use_Dense_Binning=use_dense_5d, args=args), "'mdf_clasdis'")
            mdf_clasdis = ensure_defined_col(mdf_clasdis, "Q2_y_z_pT_4D_Bins", Q2_y_z_pT_4D_Bin_Def_Function_New(Variable_Type=""), "'mdf_clasdis'")

            mdf_clasdis = ensure_z_pT_Bin_Y_bin(mdf_clasdis, "gen")
            mdf_clasdis = ensure_multidim_3d(mdf_clasdis, "MultiDim_z_pT_Bin_Y_bin_phi_t_gen", "gen", "'mdf_clasdis'")
            mdf_clasdis = ensure_defined_col(mdf_clasdis, "MultiDim_Q2_y_z_pT_phi_h_gen", Multi_Bin_Standard_Def_Function(Variable_Type="gen", Dimension="5D", Use_Dense_Binning=use_dense_5d, args=args), "'mdf_clasdis'")
            mdf_clasdis = ensure_defined_col(mdf_clasdis, "Q2_y_z_pT_4D_Bins_gen", Q2_y_z_pT_4D_Bin_Def_Function_New(Variable_Type="gen"), "'mdf_clasdis'")

            mdf_clasdis = ensure_z_pT_Bin_Y_bin(mdf_clasdis, "smear")
            mdf_clasdis = ensure_multidim_3d(mdf_clasdis, "MultiDim_z_pT_Bin_Y_bin_phi_t_smeared", "smear", "'mdf_clasdis'")
            mdf_clasdis = ensure_defined_col(mdf_clasdis, "MultiDim_Q2_y_z_pT_phi_h_smeared", Multi_Bin_Standard_Def_Function(Variable_Type="smear", Dimension="5D", Use_Dense_Binning=use_dense_5d, args=args), "'mdf_clasdis'")
            mdf_clasdis = ensure_defined_col(mdf_clasdis, "Q2_y_z_pT_4D_Bins_smeared", Q2_y_z_pT_4D_Bin_Def_Function_New(Variable_Type="smear"), "'mdf_clasdis'")
            ...
            gdf_clasdis = ensure_z_pT_Bin_Y_bin(gdf_clasdis, "")
            gdf_clasdis = ensure_multidim_3d(gdf_clasdis, "MultiDim_z_pT_Bin_Y_bin_phi_t", "", "'gdf_clasdis'")
            gdf_clasdis = ensure_defined_col(gdf_clasdis, "MultiDim_Q2_y_z_pT_phi_h", Multi_Bin_Standard_Def_Function(Variable_Type="", Dimension="5D", Use_Dense_Binning=use_dense_5d, args=args), "'gdf_clasdis'")
            gdf_clasdis = ensure_defined_col(gdf_clasdis, "Q2_y_z_pT_4D_Bins", Q2_y_z_pT_4D_Bin_Def_Function_New(Variable_Type=""), "'gdf_clasdis'")
            ...
            if(args.Use_EvGen):
                ...
                mdf_EvGen = ensure_z_pT_Bin_Y_bin(mdf_EvGen, "")
                mdf_EvGen = ensure_multidim_3d(mdf_EvGen, "MultiDim_z_pT_Bin_Y_bin_phi_t", "", None)
                mdf_EvGen = ensure_defined_col(mdf_EvGen, "MultiDim_Q2_y_z_pT_phi_h", Multi_Bin_Standard_Def_Function(Variable_Type="", Dimension="5D", Use_Dense_Binning=use_dense_5d, args=args), None)

                mdf_EvGen = ensure_z_pT_Bin_Y_bin(mdf_EvGen, "gen")
                mdf_EvGen = ensure_multidim_3d(mdf_EvGen, "MultiDim_z_pT_Bin_Y_bin_phi_t_gen", "gen", None)
                mdf_EvGen = ensure_defined_col(mdf_EvGen, "MultiDim_Q2_y_z_pT_phi_h_gen", Multi_Bin_Standard_Def_Function(Variable_Type="gen", Dimension="5D", Use_Dense_Binning=use_dense_5d, args=args), None)

                mdf_EvGen = ensure_z_pT_Bin_Y_bin(mdf_EvGen, "smear")
                mdf_EvGen = ensure_multidim_3d(mdf_EvGen, "MultiDim_z_pT_Bin_Y_bin_phi_t_smeared", "smear", None)
                mdf_EvGen = ensure_defined_col(mdf_EvGen, "MultiDim_Q2_y_z_pT_phi_h_smeared", Multi_Bin_Standard_Def_Function(Variable_Type="smear", Dimension="5D", Use_Dense_Binning=use_dense_5d, args=args), None)
                ...
                gdf_EvGen = ensure_z_pT_Bin_Y_bin(gdf_EvGen, "")
                gdf_EvGen = ensure_multidim_3d(gdf_EvGen, "MultiDim_z_pT_Bin_Y_bin_phi_t", "", None)
        ...
        # === Multi-tag weight factors: W_phys (JSON or Spline) + W_acc (HPP) -> product columns ===
        # Columns booked by enabled_weight_specs: Event_Weight_Acc, Event_Weight_JSON, Event_Weight_Spline,
        # Event_Weight_AccJSON, Event_Weight_AccSpline (plus unweighted base via exclusive_rho_weight / none).
        rho_gdf = "exclusive_rho_weight" if(gdf_clasdis.HasColumn("exclusive_rho_weight")) else "1.0"
        rho_mdf = "exclusive_rho_weight" if(mdf_clasdis.HasColumn("exclusive_rho_weight")) else "1.0"
        use_acc = bool(args.use_hpp or args.angles_only_hpp)

        # 1) Load / declare physics weight functions (mutually exclusive JSON vs Spline)
        if(args.spline_weights):
            ...
            w_phys_gdf = "ComputeSplineWeight(Q2, xB, y, z, pT, phi_t)"
            w_phys_mdf = "ComputeSplineWeight(Q2_gen, xB_gen, y_gen, z_gen, pT_gen, phi_t_gen)"
            phys_tag   = "Spline"
            ...
            with open(args.json_file) as f:
                ...
                """)
            if(args.valerii_bins):
                w_phys_gdf = "ComputeWeight(Q2_xB_Bin_Valerii, z_pT_Bin_Valerii, phi_t)"
                w_phys_mdf = "ComputeWeight(Q2_xB_Bin_Valerii_gen, z_pT_Bin_Valerii_gen, phi_t_gen)"
            else:
                w_phys_gdf = "ComputeWeight(Q2_Y_Bin, z_pT_Bin_Y_bin, phi_t)"
                w_phys_mdf = "ComputeWeight(Q2_Y_Bin_gen, z_pT_Bin_Y_bin_gen, phi_t_gen)"
            phys_tag = "JSON"
        else:
            w_phys_gdf = "1.0"
            w_phys_mdf = "1.0"
        ...
        # 2) Factor columns W_phys / W_acc_pure (accw_*) / W_acc_comb (accw_sw_* when available)
        rec_suf = "" if(getattr(args, "unsmeared", False)) else "_smeared"
        use_comb_hpp = bool(getattr(args, "hpp_input_file_spline", None))
        gdf_clasdis = gdf_clasdis.Define("W_phys", w_phys_gdf)
        mdf_clasdis = mdf_clasdis.Define("W_phys", w_phys_mdf)
        if(use_acc):
            if(args.angles_only_hpp):
                mdf_clasdis = mdf_clasdis.Define("W_acc_pure", f"(accw_elPhi_vs_pipPhi(elPhi{rec_suf}, pipPhi{rec_suf})) * (accw_elth_vs_pipth(elth{rec_suf}, pipth{rec_suf}))")
                if(use_comb_hpp):
                    mdf_clasdis = mdf_clasdis.Define("W_acc_comb", f"(accw_sw_elPhi_vs_pipPhi(elPhi{rec_suf}, pipPhi{rec_suf})) * (accw_sw_elth_vs_pipth(elth{rec_suf}, pipth{rec_suf}))")
                else:
                    mdf_clasdis = mdf_clasdis.Define("W_acc_comb", "W_acc_pure")
            else:
                mdf_clasdis = mdf_clasdis.Define("W_acc_pure", f"(accw_elPhi_vs_pipPhi(elPhi{rec_suf}, pipPhi{rec_suf})) * (accw_elth_vs_pipth(elth{rec_suf}, pipth{rec_suf})) * (accw_el_vs_pip(el{rec_suf}, pip{rec_suf}))")
                if(use_comb_hpp):
                    mdf_clasdis = mdf_clasdis.Define("W_acc_comb", f"(accw_sw_elPhi_vs_pipPhi(elPhi{rec_suf}, pipPhi{rec_suf})) * (accw_sw_elth_vs_pipth(elth{rec_suf}, pipth{rec_suf})) * (accw_sw_el_vs_pip(el{rec_suf}, pip{rec_suf}))")
                else:
                    mdf_clasdis = mdf_clasdis.Define("W_acc_comb", "W_acc_pure")
            mdf_clasdis = mdf_clasdis.Define("W_acc", "W_acc_pure")
        else:
            mdf_clasdis = mdf_clasdis.Define("W_acc_pure", "1.0")
            mdf_clasdis = mdf_clasdis.Define("W_acc_comb", "1.0")
            mdf_clasdis = mdf_clasdis.Define("W_acc", "1.0")

        # 3) Physics-only product columns (no acceptance renorm)
        if(phys_tag in ["JSON"]):
            gdf_clasdis = gdf_clasdis.Define("Event_Weight_JSON", f"{rho_gdf} * W_phys")
            mdf_clasdis = mdf_clasdis.Define("Event_Weight_JSON", f"{rho_mdf} * W_phys")
        if(phys_tag in ["Spline"]):
            gdf_clasdis = gdf_clasdis.Define("Event_Weight_Spline", f"{rho_gdf} * W_phys")
            mdf_clasdis = mdf_clasdis.Define("Event_Weight_Spline", f"{rho_mdf} * W_phys")

        # Legacy Event_Weight on gdf (rho * phys or rho) for any residual single-column paths
        gdf_clasdis = gdf_clasdis.Define("Event_Weight", f"{rho_gdf} * W_phys")
        mdf_clasdis = mdf_clasdis.Define("Event_Weight", f"{rho_mdf} * W_phys")
        ...
        if(args.Use_EvGen):
            if(args.spline_weights):
                ...
                mdf_EvGen = mdf_EvGen.Define("Event_Weight_Spline", "weight*ComputeSplineWeight(Q2, xB, y, z, pT, phi_t)")
                gdf_EvGen = gdf_EvGen.Define("Event_Weight_Spline", "weight*ComputeSplineWeight(Q2_gen, xB_gen, y_gen, z_gen, pT_gen, phi_t_gen)")
            elif(args.json_weights):
                if(args.valerii_bins):
                    ...
                    mdf_EvGen = mdf_EvGen.Define("Event_Weight_JSON", "weight*ComputeWeight(Q2_xB_Bin_Valerii_gen, z_pT_Bin_Valerii_gen, phi_t_gen)")
                    gdf_EvGen = gdf_EvGen.Define("Event_Weight_JSON", "weight*ComputeWeight(Q2_xB_Bin_Valerii, z_pT_Bin_Valerii, phi_t)")
                else:
                    ...
                    mdf_EvGen = mdf_EvGen.Define("Event_Weight_JSON", "weight*ComputeWeight(Q2_Y_Bin_gen, z_pT_Bin_Y_bin_gen, phi_t_gen)")
                    gdf_EvGen = gdf_EvGen.Define("Event_Weight_JSON", "weight*ComputeWeight(Q2_Y_Bin, z_pT_Bin_Y_bin, phi_t)")
            else:
                mdf_EvGen = mdf_EvGen.Define("Event_Weight", "weight")
                gdf_EvGen = gdf_EvGen.Define("Event_Weight", "weight")

        Histograms_All = {}
        # 4) Acceptance (+ AccPhys) renormed product columns on mdf
        do_not_use_smeared = bool(getattr(args, "unsmeared", False))
        if(use_acc):
            # Acc-only: renorm with W_pre = 1 and pure HPP (W_acc_pure)
            mdf_clasdis = mdf_clasdis.Redefine("W_pre", "1.0") if(mdf_clasdis.HasColumn("W_pre")) else mdf_clasdis.Define("W_pre", "1.0")
            mdf_clasdis = mdf_clasdis.Redefine("W_acc", "W_acc_pure")
            mdf_clasdis = mdf_clasdis.Redefine("Event_Weight_raw", "W_pre * W_acc") if(mdf_clasdis.HasColumn("Event_Weight_raw")) else mdf_clasdis.Define("Event_Weight_raw", "W_pre * W_acc")
            if(args.use_hpp):
                mdf_clasdis, args, Additional_Histos = weight_norm_by_bins_wHisto(df_in=mdf_clasdis, Histo_Data_In="mdf", args=args, Do_not_use_Smeared=do_not_use_smeared, Valerii_binning=args.valerii_bins, target_event_weight_col="Event_Weight_Acc")
                ...
                # angles_only without full use_hpp: Acc product without bin renorm
                mdf_clasdis = mdf_clasdis.Define("Event_Weight_Acc", f"{rho_mdf} * W_acc")
            # AccPhys: renorm with W_pre = W_phys and combined HPP (W_acc_comb / accw_sw_*)
            if(phys_tag is not None):
                acc_phys_col = f"Event_Weight_Acc{phys_tag}"
                mdf_clasdis  = mdf_clasdis.Redefine("W_pre", "W_phys")
                mdf_clasdis  = mdf_clasdis.Redefine("W_acc", "W_acc_comb")
                mdf_clasdis  = mdf_clasdis.Redefine("Event_Weight_raw", "W_pre * W_acc")
                if(args.use_hpp):
                    mdf_clasdis, args, Additional_Histos = weight_norm_by_bins_wHisto(df_in=mdf_clasdis, Histo_Data_In="mdf", args=args, Do_not_use_Smeared=do_not_use_smeared, Valerii_binning=args.valerii_bins, target_event_weight_col=acc_phys_col)
                    if(isinstance(Additional_Histos, str)):
                        Crash_Report(args, crash_message=f"{Additional_Histos}\n{color.END_B}Will Continue Running Anyway...{color.END}", continue_run=True)
                    elif(isinstance(Additional_Histos, dict)):
                        if((len(Additional_Histos) > 0)):
                    ...
                    mdf_clasdis = mdf_clasdis.Define(acc_phys_col, f"{rho_mdf} * W_phys * W_acc")
                # Keep legacy Event_Weight as the AccPhys product when both are active
                if(mdf_clasdis.HasColumn(acc_phys_col)):
                    mdf_clasdis = mdf_clasdis.Redefine("Event_Weight", acc_phys_col)
            else:
                if(mdf_clasdis.HasColumn("Event_Weight_Acc")):
                    mdf_clasdis = mdf_clasdis.Redefine("Event_Weight", "Event_Weight_Acc")
        else:
            mdf_clasdis = mdf_clasdis.Define("W_pre", "W_phys") if(not mdf_clasdis.HasColumn("W_pre")) else mdf_clasdis.Redefine("W_pre", "W_phys")
            mdf_clasdis = mdf_clasdis.Define("Event_Weight_raw", "W_pre * W_acc") if(not mdf_clasdis.HasColumn("Event_Weight_raw")) else mdf_clasdis.Redefine("Event_Weight_raw", "W_pre * W_acc")

        weight_specs_mdf = enabled_weight_specs(args, "mdf")
        weight_specs_gdf = enabled_weight_specs(args, "gdf")
        mdf_smear_type   = "" if(getattr(args, "unsmeared", False)) else "smear"
        print(f"{color.BLUE}Will be saving to: {color.BGREEN}{args.root}{color.END}")
        print(f"{color.BBLUE}Weight tags (mdf): {color.END_B}{[tag for tag, col in weight_specs_mdf]}{color.END}")
        print(f"{color.BBLUE}Weight tags (gdf): {color.END_B}{[tag for tag, col in weight_specs_gdf]}{color.END}")
        if(getattr(args, "unsmeared", False)):
            print(f"{color.BBLUE}Unsmeared mode: mdf Histo_Smear=''{color.END}")
        sys.stdout.flush()
        Res_Binning_2D_z_pT_In     = ["z_pT_Bin_Y_bin_smeared",           -0.5,     37.5,    38] if(not getattr(args, "unsmeared", False)) else ["z_pT_Bin_Y_bin", -0.5, 37.5, 38]
        z_pT_phi_h_Binning         = ['MultiDim_z_pT_Bin_Y_bin_phi_t',    -1.5,    913.5,   915]
        dense_3d_map               = None
        if(use_dense_3d):
            dense_3d_map = getattr(args, "_dense_3d_mapping", None)
            if(dense_3d_map is None):
        ...
        if(args.valerii_bins):
            ...
            dense_3d_map           = None
            phi_h_5D_Binning       = ['Q2_xB_z_pT_phi_t_5D_Bin_Valerii',  -0.5,  11520.5, 11521]
            Sliced_5D_Increment    = 281 # Gives 41 slices to form the full response matrix
        ...
        if(not (args.make_2D_only or args.unfold_5D_only)):
            for Q2_y_Bins in Q2_y_or_xB_bin_range:
                if(Q2_y_Bins == 0):
                    continue
                if(dense_3d_map is not None):
                    z_pT_phi_h_Binning = dense_3d_var_input(Q2_y_Bins, dense_3d_map)
        ...
        if(args.unfold_5D and (not args.make_2D_only)):
            ...
            if(args.Use_EvGen):
                # EvGen has no HPP Acc columns; use phys-only specs (same as gdf)
                Histograms_All = make_rm5d_single(sdf=mdf_EvGen,   Histo_Group="Response_Matrix_Normal",     Histo_Data="mdf", Histo_Cut=f"{args.cut_name_mdf}{'' if(args.cut_rho0 in ['']) else f'_{args.cut_rho0}'}{'' if(not (args.cut or args.cut_MC))   else '_Extra'}",     Histo_Smear=mdf_smear_type, Binning="Y_bin" if(not args.valerii_bins) else "Valerii", Q2_y_z_pT_phi_h_5D_Binning=phi_h_5D_Binning,          Use_Weight=False, Sliced_5D_Increment=Sliced_5D_Increment, Histograms_All=Histograms_All, custom_title=args.title, weight_specs=weight_specs_gdf)
                ...
                Histograms_All = make_rm5d_single(sdf=gdf_EvGen,   Histo_Group="Response_Matrix_Normal",     Histo_Data="gdf", Histo_Cut=f"{args.cut_name_gdf}{'' if(args.cut_rho0 in ['']) else f'_{args.cut_rho0}'}{'' if(not (args.cut or args.cut_MC))   else '_Extra'}",     Histo_Smear="",      Binning="Y_bin" if(not args.valerii_bins) else "Valerii", Q2_y_z_pT_phi_h_5D_Binning=phi_h_5D_Binning,          Use_Weight=False, Sliced_5D_Increment=Sliced_5D_Increment, Histograms_All=Histograms_All, custom_title=args.title, weight_specs=weight_specs_gdf)
        ...
        if(args.make_2D):
            ...
            if(str(args.z_axis_2D) in ["4D_Bin", "Q2_Y_Bin"]):
                ...
                if(str(args.z_axis_2D) not in ["4D_Bin"]):
                ...
                List_of_2D_Plots.append([["z1_plus_z2", 0, 1.8, 180], ["MM_pippim",                  0.0,   4.5,  90]])
                List_of_2D_Plots.append([["W_pippim", 0.0, 2.5, 100], ["MM_pippim",                  0.0,   4.5,  90]])
                List_of_2D_Plots.append([["z_rho",      0, 1.8, 180], ["W_pippim",                   0.0,   2.5, 100]])
            ...
            if(args.Use_EvGen):
                for Vars in List_of_2D_Plots:
                    ...
                    Histograms_All = make_TH2D_histos(sdf=mdf_EvGen, Histo_Data="mdf", Histo_Cut=f"{args.cut_name_mdf}{'' if(args.cut_rho0 in ['']) else f'_{args.cut_rho0}'}{'' if(not (args.cut or args.cut_MC)) else '_Extra'}", Histo_Smear="", Binning="Y_bin" if(not args.valerii_bins) else "Valerii", Vars_Input=Vars, Use_Weight=False, Histograms_All=Histograms_All, Histo_Group="Normal_2D", custom_title=args.title, args_in=args, axis_Z=args.z_axis_2D, weight_specs=weight_specs_gdf)
                    Histograms_All = make_TH2D_histos(sdf=gdf_EvGen, Histo_Data="gdf", Histo_Cut=f"{args.cut_name_gdf}{'' if(args.cut_rho0 in ['']) else f'_{args.cut_rho0}'}{'' if(not (args.cut or args.cut_MC)) else '_Extra'}", Histo_Smear="", Binning="Y_bin" if(not args.valerii_bins) else "Valerii", Vars_Input=Vars, Use_Weight=False, Histograms_All=Histograms_All, Histo_Group="Normal_2D", custom_title=args.title, args_in=args, axis_Z=args.z_axis_2D, weight_specs=weight_specs_gdf)
