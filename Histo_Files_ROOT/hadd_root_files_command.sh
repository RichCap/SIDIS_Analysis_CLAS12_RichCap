...
case "$OPTION" in
    rdf)
        ...
        INPUT_FILES="${DIRECTORY}${FILE_PATTERN}_5*root"
        ;;
    mdf)
        ...
        INPUT_FILES="${DIRECTORY}${FILE_PATTERN}_3*root"
        ;;
    gdf)
...
if [ "$ACTION" = "check" ]; then
    ...
    echo ""
    $ROOTSYS/bin/hadd ${OUTPUT_FILE} ${INPUT_FILES}; ls -lhtr ${DIRECTORY}${FILE_PATTERN}*; ls -lhtr ${OUTPUT_FILE}
    echo "Done running the hadd command."
    echo ""
    # Call cleanup script only if hadd was successful
    if [ $? -eq 0 ]; then
        ./cleanup_root_files.sh "$DIRECTORY" "$FILE_PATTERN" "$OPTION"
    fi
    echo ""
fi
