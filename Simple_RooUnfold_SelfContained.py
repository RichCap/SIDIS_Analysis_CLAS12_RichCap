...
def parse_args():
    ...
    p.add_argument('-r', '--root',
                   type=str,
                   default="Unfolded_Histos_From_Just_RooUnfold_SIDIS_richcap.root",
...
print(f"\n{color.BBLUE}Smear option selected is: {'No Smear' if(str(Smearing_Options) in ['', 'no_smear']) else str(Smearing_Options.replace('_s', 'S')).replace('s', 'S')}{color.END}\n")
...
def Histogram_Name_Def(out_print, Histo_General="Find", Data_Type="Find", Cut_Type="Find", Smear_Type="Find", Q2_y_Bin="All", z_pT_Bin="All", Bin_Extra="Default", Variable="Default"):
    ...
    if((Closure_Test or Mod_Test or Sim_Test) and (")_(" in str(Name_Output)) and all(extra not in str(Name_Output) for extra in ["Mod_Test", "Closure_Test", "Sim_Test"])):
...
email_body = f"""
The 'Simple_RooUnfold_SelfContained.py' script has finished running.
{start_time}
...
if(args.email):
    send_email(subject="Finished Running the 'Simple_RooUnfold_SelfContained.py' Code", body=email_body, recipient="richard.capobianco@uconn.edu")
