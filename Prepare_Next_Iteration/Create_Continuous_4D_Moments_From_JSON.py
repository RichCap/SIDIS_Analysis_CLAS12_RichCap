...
def generate_initial_dict(info_map, args):
    ...
    for q2y_bin, zpt_bin in bin_pairs:
        ...
        fit_set_str = str(getattr(args, "fit_set", ""))
        dim_tag = "5D" if("5D" in fit_set_str) else "3D" if("3D" in fit_set_str) else ""
        fam_tag = "BC" if("BC_RC" in fit_set_str) else "RC" if("RC_" in fit_set_str) else ""
        key_parts = [f'"{q2y_bin}"', f'"{zpt_bin}"']
        if(dim_tag not in [""]):
            key_parts.append(f'"{dim_tag}"')
        if(fam_tag not in [""]):
            key_parts.append(f'"{fam_tag}"')
        entry = f'''    ({", ".join(key_parts)}): {{
        "B_initial": {B_init:.5f},
    ...
    kept_lines = []
    out_name_probe = "Phi_h_Fit_Parameters_from_Spline.py"
    save_init_probe = getattr(args, "save_init", None)
    if((save_init_probe is not None) and str(save_init_probe).strip().endswith(".py")):
        out_name_probe = str(save_init_probe).strip()
    if(os.path.isfile(out_name_probe)):
        try:
            import importlib.util
            spec = importlib.util.spec_from_file_location("existing_phi_h_init", out_name_probe)
            existing_mod = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(existing_mod)
            existing_set = getattr(existing_mod, "special_fit_parameters_set", {})
            fit_set_str = str(getattr(args, "fit_set", ""))
            dim_tag = "5D" if("5D" in fit_set_str) else "3D" if("3D" in fit_set_str) else ""
            fam_tag = "BC" if("BC_RC" in fit_set_str) else "RC" if("RC_" in fit_set_str) else ""
            for key, settings in existing_set.items():
                if((not isinstance(key, tuple)) or (len(key) < 2)):
                    continue
                if(any(str(part) in ["Trusted", "Sectors"] for part in key)):
                    continue
                key_tags = [str(part) for part in key[2:]]
                this_family = (key_tags == ([dim_tag, fam_tag] if((dim_tag not in [""]) and (fam_tag not in [""])) else [dim_tag] if(dim_tag not in [""]) else [fam_tag] if(fam_tag not in [""]) else []))
                if(this_family):
                    continue
                if(not isinstance(settings, dict)):
                    continue
                if("B_initial" not in settings):
                    continue
                key_s = ", ".join(f'"{part}"' for part in key)
                am = settings.get("Allow_Multiple_Fits", True)
                amc = settings.get("Allow_Multiple_Fits_C", True)
                b_lim = settings.get("B_limits", [0, 0])
                c_lim = settings.get("C_limits", [0, 0])
                kept_lines.append(f'''    ({key_s}): {{
        "B_initial": {float(settings.get("B_initial", 0.0)):.5f},
        "B_limits":  [{float(b_lim[0]):.4f}, {float(b_lim[1]):.4f}],
        "C_initial": {float(settings.get("C_initial", 0.0)):.5f},
        "C_limits":  [{float(c_lim[0]):.4f}, {float(c_lim[1]):.4f}],
        "Allow_Multiple_Fits":   {am},
        "Allow_Multiple_Fits_C": {amc}
    }},''')
        except Exception as merge_err:
            print(f"{color.BYELLOW}WARNING:{color.END} Could not merge existing initializer '{out_name_probe}': {merge_err}")
            kept_lines = []
...
# SciPy RBFInterpolator (farm target SciPy 1.16.2): dual coeffs; kernel on eps*x; poly on (x-shift)/scale.
# Do not export training targets as dual coefficients (that caused all-ones withSpline HPPs).
# ------------------------------------------------------------
class ExpRBFWrapper:
...
def _require_rbf_export_attrs(rbf, y_par):
    # Fail clearly if SciPy RBFInterpolator lacks the structure needed for C++ export.
    required = ["y", "_coeffs", "_shift", "_scale", "powers", "kernel", "epsilon", "neighbors"]
    missing = [a for a in required if(not hasattr(rbf, a))]
    if(missing):
        scipy_ver = getattr(__import__("scipy"), "__version__", "?")
        raise RuntimeError(f"[{y_par}] SciPy RBFInterpolator (scipy {scipy_ver}) missing required export attributes {missing}. Farm target is SciPy 1.16.2; refuse legacy sum(y_i*K) export.")
    if(getattr(rbf, "neighbors", None) is not None):
        raise RuntimeError(f"[{y_par}] RBFInterpolator.neighbors={rbf.neighbors!r} is not None; global dual-coeff C++ export is not valid for the neighbor evaluation path.")
    P = int(np.asarray(rbf.y).shape[0])
    R = int(np.asarray(rbf.powers).shape[0])
    coeffs = np.asarray(rbf._coeffs, dtype=float)
    if(coeffs.ndim == 2):
        if(coeffs.shape[1] != 1):
            raise RuntimeError(f"[{y_par}] expected single-output coeffs, got shape {coeffs.shape}")
        coeffs = coeffs.reshape(-1)
    if(int(coeffs.shape[0]) != P + R):
        raise RuntimeError(f"[{y_par}] coeffs length {coeffs.shape[0]} != n_points+n_poly ({P}+{R}={P+R})")
    return P, R, coeffs

def _rebuild_rbf_from_npz_record(rec, y_par):
    # Rebuild RBFInterpolator from NPZ fields (portable across pickle/SciPy versions).
    points = np.asarray(rec["points"], dtype=float)
    values = np.asarray(rec["values"], dtype=float)  # fit targets (log(A) or B/C)
    kernel = str(rec["kernel"])
    smoothing = float(rec["smoothing"])
    epsilon = float(rec["epsilon"])
    # Match training call site: no explicit degree/neighbors -> SciPy defaults for this kernel.
    rbf = RBFInterpolator(points, values, kernel=kernel, smoothing=smoothing, epsilon=epsilon)
    _require_rbf_export_attrs(rbf, y_par)
    return rbf
...
def generate_spline_code(args):
    ...
    scipy_ver = __import__("scipy").__version__
    print(f"{color.BBLUE}SciPy version in this process: {scipy_ver} (farm production target: 1.16.2; export uses public rebuild + private dual coeffs){color.END}")
    files = {}
    ...
    # Load NPZ metadata + rebuild RBF to obtain dual coefficients (not training targets).
    data = {}
    for y_par, fname in files.items():
        ...
        rec = {
            ...
            "epsilon": float(d["epsilon"]) if("epsilon" in d.files) else 1.0,
            "dimension_mode": str(d["dimension_mode"]),
            "log_space": bool(d["log_space"]) if("log_space" in d.files) else False,
        }
        rbf = _rebuild_rbf_from_npz_record(rec, y_par)
        P, R, coeffs = _require_rbf_export_attrs(rbf, y_par)
        data[y_par] = {
            **rec,
            "rbf": rbf,
            "n_points": P,
            "n_poly": R,
            "coeffs": coeffs,
            "powers": np.asarray(rbf.powers, dtype=int),
            "shift": np.asarray(rbf._shift, dtype=float).reshape(-1),
            "scale": np.asarray(rbf._scale, dtype=float).reshape(-1),
        }
        print(f"  {y_par}: P={P} R={R} kernel={rec['kernel']} eps={rec['epsilon']} smooth={rec['smoothing']} log_space={rec['log_space']} coeffs_len={len(coeffs)}")

    gen_time = __import__("datetime").datetime.now().strftime("%m-%d-%Y %H:%M:%S")
    # SciPy 1.16.2-faithful kernels (epsilon applied via coordinate scaling, not inside these).
    cpp_code = ""
    cpp_code += "// Auto-generated by Create_Continuous_4D_Moments_From_JSON.py\n"
    cpp_code += f"// Dimension mode: {args.dimension_mode}\n"
    cpp_code += f"// Fit set: {args.fit_set}\n"
    cpp_code += f"// Generated on {gen_time}\n"
    cpp_code += "// Evaluator matches SciPy RBFInterpolator (target farm SciPy 1.16.2):\n"
    cpp_code += "//   - dual coefficients (not training targets)\n"
    cpp_code += "//   - kernel on epsilon-scaled original coordinates\n"
    cpp_code += "//   - polynomial on (x-shift)/scale only\n"
    cpp_code += "#include <cmath>\n"
    cpp_code += "#include <string>\n"
    cpp_code += "#include <TMath.h>\n\n"
    cpp_code += "// SciPy 1.16.2 NAME_TO_FUNC kernels (distance already epsilon-scaled by caller).\n"
    cpp_code += "inline double scipy_rbf_kernel(const double r, const std::string& kernel) {\n"
    cpp_code += "    if(kernel == \"thin_plate_spline\") {\n"
    cpp_code += "        if(r == 0.0) { return 0.0; }\n"
    cpp_code += "        return r*r * std::log(r);\n"
    cpp_code += "    }\n"
    cpp_code += "    if(kernel == \"linear\") { return -r; }\n"
    cpp_code += "    if(kernel == \"cubic\") { return r*r*r; }\n"
    cpp_code += "    if(kernel == \"quintic\") { return -r*r*r*r*r; }\n"
    cpp_code += "    if(kernel == \"multiquadric\") { return -std::sqrt(r*r + 1.0); }\n"
    cpp_code += "    if(kernel == \"inverse_multiquadric\") { return 1.0 / std::sqrt(r*r + 1.0); }\n"
    cpp_code += "    if(kernel == \"inverse_quadratic\") { return 1.0 / (r*r + 1.0); }\n"
    cpp_code += "    if(kernel == \"gaussian\") { return std::exp(-r*r); }\n"
    cpp_code += "    return r;\n"
    cpp_code += "}\n"
    ...
    for y_par in ["Fit_Par_A", "Fit_Par_B", "Fit_Par_C"]:
        ...
        log_space = bool(d["log_space"])
        cpp_code += f"\n// --- {y_par} data (dual-coeff RBF export) ---\n"
        cpp_code += f"const int n_{tag} = {P};\n"
        cpp_code += f"const int n_poly_{tag} = {R};\n"
        cpp_code += f"const int ndim_{tag} = {dim};\n"
        cpp_code += f"const double centers_{tag}[{P}][{dim}] = {{\n"
        for p in d["points"]:
            cpp_code += "    {" + ", ".join(f"{float(x):.16e}" for x in p) + "},\n"
        cpp_code += "};\n"
        cpp_code += f"const double coeffs_{tag}[{P + R}] = {{{', '.join(f'{float(v):.16e}' for v in d['coeffs'])}}};\n"
        cpp_code += f"const int powers_{tag}[{R}][{dim}] = {{\n"
        for row in d["powers"]:
            cpp_code += "    {" + ", ".join(f"{int(v)}" for v in row) + "},\n"
        cpp_code += "};\n"
        cpp_code += f"const double shift_{tag}[{dim}] = {{{', '.join(f'{float(v):.16e}' for v in d['shift'])}}};\n"
        cpp_code += f"const double scale_{tag}[{dim}] = {{{', '.join(f'{float(v):.16e}' for v in d['scale'])}}};\n"
        cpp_code += f"const std::string kernel_{tag} = \"{d['kernel']}\";\n"
        cpp_code += f"const double epsilon_{tag} = {float(d['epsilon']):.16e};\n"
        cpp_code += f"const std::string dim_mode_{tag} = \"{d['dimension_mode']}\";\n"
        cpp_code += f"const bool log_space_{tag} = {'true' if(log_space) else 'false'};\n"
        cpp_code += f"\ndouble ComputeSpline{y_par[-1]}(double Q2, double xB, double y, double z, double pT) {{\n"
        cpp_code += "    double pt[5];\n"
        cpp_code += f"    if(dim_mode_{tag} == \"4D\") {{ pt[0] = Q2; pt[1] = y; pt[2] = z; pt[3] = pT; pt[4] = 0.0; }}\n"
        cpp_code += f"    else if(dim_mode_{tag} == \"4D_xB\") {{ pt[0] = xB; pt[1] = y; pt[2] = z; pt[3] = pT; pt[4] = 0.0; }}\n"
        cpp_code += f"    else {{ pt[0] = Q2; pt[1] = xB; pt[2] = y; pt[3] = z; pt[4] = pT; }} // 5D\n"
        cpp_code += f"    const int ndim = ndim_{tag};\n"
        cpp_code += f"    const int P = n_{tag};\n"
        cpp_code += f"    const int R = n_poly_{tag};\n"
        cpp_code += f"    const double eps = epsilon_{tag};\n"
        cpp_code += "    double sum = 0.0;\n"
        cpp_code += "    // Kernel block: distances on epsilon-scaled original coordinates (SciPy 1.16.2).\n"
        cpp_code += "    for(int i = 0; i < P; ++i) {\n"
        cpp_code += "        double r2 = 0.0;\n"
        cpp_code += "        for(int j = 0; j < ndim; ++j) {\n"
        cpp_code += f"            const double diff = eps * pt[j] - eps * centers_{tag}[i][j];\n"
        cpp_code += "            r2 += diff * diff;\n"
        cpp_code += "        }\n"
        cpp_code += "        const double r = std::sqrt(r2);\n"
        cpp_code += f"        sum += coeffs_{tag}[i] * scipy_rbf_kernel(r, kernel_{tag});\n"
        cpp_code += "    }\n"
        cpp_code += "    // Polynomial block: shift/scale only (not applied to kernel distances).\n"
        cpp_code += "    double xhat[5];\n"
        cpp_code += "    for(int j = 0; j < ndim; ++j) {\n"
        cpp_code += f"        xhat[j] = (pt[j] - shift_{tag}[j]) / scale_{tag}[j];\n"
        cpp_code += "    }\n"
        cpp_code += "    for(int m = 0; m < R; ++m) {\n"
        cpp_code += "        double mono = 1.0;\n"
        cpp_code += "        for(int j = 0; j < ndim; ++j) {\n"
        cpp_code += f"            const int powv = powers_{tag}[m][j];\n"
        cpp_code += "            if(powv != 0) {\n"
        cpp_code += "                mono *= std::pow(xhat[j], powv);\n"
        cpp_code += "            }\n"
        cpp_code += "        }\n"
        cpp_code += f"        sum += coeffs_{tag}[P + m] * mono;\n"
        cpp_code += "    }\n"
        cpp_code += f"    if(log_space_{tag}) {{ return std::exp(sum); }}\n"
        cpp_code += "    return sum;\n"
        cpp_code += "}\n"

    # Main weight function (JSON-compatible form; phi in degrees)
    cpp_code += "\ndouble ComputeSplineWeight(double Q2, double xB, double y, double z, double pT, double phi_h) {\n"
    cpp_code += "    const double phi_rad = phi_h * TMath::DegToRad();\n"
    cpp_code += "    const double A = ComputeSplineA(Q2, xB, y, z, pT);\n"
    cpp_code += "    const double B = ComputeSplineB(Q2, xB, y, z, pT);\n"
    cpp_code += "    const double C = ComputeSplineC(Q2, xB, y, z, pT);\n"
    cpp_code += "    return A * (1.0 + B * std::cos(phi_rad) + C * std::cos(2.0 * phi_rad));\n"
    cpp_code += "}\n"
