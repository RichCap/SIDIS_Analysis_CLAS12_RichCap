...
// Reconstructed PID Cut variations:
    // Electrons
branches_string += ':Full_norm_el:Full_tight_el:Full_mid_el:Full_loose_el'
branches_string += ':DC_VERTEX_norm_el:DC_VERTEX_tight_el:DC_VERTEX_mid_el:DC_VERTEX_loose_el'
branches_string += ':DC_FIDUCIAL_REG_norm_el:DC_FIDUCIAL_REG_tight_el:DC_FIDUCIAL_REG_mid_el:DC_FIDUCIAL_REG_loose_el'
branches_string += ':DC_FIDUCIAL_REG3_norm_el:DC_FIDUCIAL_REG3_tight_el:DC_FIDUCIAL_REG3_mid_el:DC_FIDUCIAL_REG3_loose_el'
branches_string += ':DC_FIDUCIAL_REG2_norm_el:DC_FIDUCIAL_REG2_tight_el:DC_FIDUCIAL_REG2_mid_el:DC_FIDUCIAL_REG2_loose_el'
branches_string += ':DC_FIDUCIAL_REG1_norm_el:DC_FIDUCIAL_REG1_tight_el:DC_FIDUCIAL_REG1_mid_el:DC_FIDUCIAL_REG1_loose_el'
branches_string += ':EC_FIDUCIAL_norm_el:EC_FIDUCIAL_tight_el:EC_FIDUCIAL_mid_el:EC_FIDUCIAL_loose_el'
branches_string += ':EC_SAMPLING_norm_el:EC_SAMPLING_tight_el:EC_SAMPLING_mid_el:EC_SAMPLING_loose_el'
branches_string += ':EC_OUTER_VS_INNER_norm_el:EC_OUTER_VS_INNER_tight_el:EC_OUTER_VS_INNER_mid_el:EC_OUTER_VS_INNER_loose_el'
branches_string += ':CC_NPHE_norm_el:PID_norm_el'
    // Pi+ Pions
branches_string += ':Full_norm_pip:Full_tight_pip:Full_mid_pip:Full_loose_pip'
branches_string += ':DELTA_VZ_norm_pip:DELTA_VZ_tight_pip:DELTA_VZ_mid_pip:DELTA_VZ_loose_pip'
branches_string += ':DC_FIDUCIAL_REG_norm_pip:DC_FIDUCIAL_REG_tight_pip:DC_FIDUCIAL_REG_mid_pip:DC_FIDUCIAL_REG_loose_pip'
branches_string += ':DC_FIDUCIAL_REG3_norm_pip:DC_FIDUCIAL_REG3_tight_pip:DC_FIDUCIAL_REG3_mid_pip:DC_FIDUCIAL_REG3_loose_pip'
branches_string += ':DC_FIDUCIAL_REG2_norm_pip:DC_FIDUCIAL_REG2_tight_pip:DC_FIDUCIAL_REG2_mid_pip:DC_FIDUCIAL_REG2_loose_pip'
branches_string += ':DC_FIDUCIAL_REG1_norm_pip:DC_FIDUCIAL_REG1_tight_pip:DC_FIDUCIAL_REG1_mid_pip:DC_FIDUCIAL_REG1_loose_pip'
branches_string += ':CHI2PID_CUT_norm_pip:CHI2PID_CUT_tight_pip:CHI2PID_CUT_mid_pip:CHI2PID_CUT_loose_pip'
branches_string += ':FORWARD_norm_pip:PID_norm_pip'
...
def Custom_EC_OUTER_VS_INNER(def eleCan_In, def cutLevel_In) {
    float pcal_energy  = eleCan_In.pcal_energy;
...
def Custom_EC_SAMPLING(def eleCan_In, def cutLevel_In) {
    int pcal_sector    = eleCan_In.getPCALsector();
    float partp        = eleCan_In.p;
    float pcal_energy  = eleCan_In.pcal_energy;
    float ecin_energy  = eleCan_In.ecin_energy;
    float ecout_energy = eleCan_In.ecout_energy;
...
def Custom_EC_FIDUCIAL(def eleCan_In, def cutLevel_In) {
    int pcal_sector = eleCan_In.getPCALsector();
    float lv        = eleCan_In.pcal_lv;
    float lw        = eleCan_In.pcal_lw;
...
def Custom_DC_FIDUCIAL_REG(def eleCan_In, def cutLevel_In, def region, boolean isinbending=isinb) {
    int dc_sector = eleCan_In.getDCsector();
    float x       = eleCan_In.getDC1x(); // traj_x1
    float y       = eleCan_In.getDC1y(); // traj_y1
    if(region == 2){
    ...
    int partpid   = eleCan_In.pid; // Should always be 11 for electron
...
def Custom_DC_VERTEX(def eleCan_In, def cutLevel_In, boolean isinbending=isinb) {
    int pcal_sector    = eleCan_In.getPCALsector();
    float partvz       = eleCan_In.vz;
    ...
    double level_var = 0;
    if(cutLevel_In == 'loose') { level_var = -0.5;}
    if(cutLevel_In == 'tight') { level_var =  0.5;}

    for(int i = 0; i < 6; i++) {
        if(isinbending) {
            vz_min_sect[i] = vz_min_sect_inb[i] + level_var;
            vz_max_sect[i] = vz_max_sect_inb[i] - level_var;
        } else {
            vz_min_sect[i] = vz_min_sect_outb[i] + level_var;
            vz_max_sect[i] = vz_max_sect_outb[i] - level_var;
        }
    }
...
def Custom_CHI2PID_CUT_pip(def pipCan_In, def cutLevel_In) {
    double p = pipCan_In.p;
    int pid  = pipCan_In.pid; 
    double chi2pid = pipCan_In.chi2pid;
...
def Custom_DC_FIDUCIAL_REG_pip(def pipCan_In, def cutLevel_In, def region, boolean isinbending=isinb) {
    int dc_sector = pipCan_In.getDCsector();
    float trajx  = pipCan_In.getDC1x(); // traj_x1
    float trajy  = pipCan_In.getDC1y(); // traj_y1
    float trajz  = pipCan_In.getDC1z(); // traj_z1
    if(region == 2) {
                ...
                for(int ipart = 1; ipart < partb.getRows(); ipart++){
                    ...
                    if(pip_pion_PIDs.Full_norm || pip_pion_PIDs.Full_tight || pip_pion_PIDs.Full_mid || pip_pion_PIDs.Full_loose || canpip.ispip()){
                        ...
                        tt.fill(evn,      run,      beamCharge,        ex, ey, ez,        pipx, pipy, pipz,
                                ...
                                // Electron PID Cuts
                                electron_PIDs.Full_norm,              electron_PIDs.Full_tight,              electron_PIDs.Full_mid,              electron_PIDs.Full_loose,
                                electron_PIDs.DC_VERTEX_norm,         electron_PIDs.DC_VERTEX_tight,         electron_PIDs.DC_VERTEX_mid,         electron_PIDs.DC_VERTEX_loose,
                                electron_PIDs.DC_FIDUCIAL_REG_norm,   electron_PIDs.DC_FIDUCIAL_REG_tight,   electron_PIDs.DC_FIDUCIAL_REG_mid,   electron_PIDs.DC_FIDUCIAL_REG_loose,
                                electron_PIDs.DC_FIDUCIAL_REG3_norm,  electron_PIDs.DC_FIDUCIAL_REG3_tight,  electron_PIDs.DC_FIDUCIAL_REG3_mid,  electron_PIDs.DC_FIDUCIAL_REG3_loose,
                                electron_PIDs.DC_FIDUCIAL_REG2_norm,  electron_PIDs.DC_FIDUCIAL_REG2_tight,  electron_PIDs.DC_FIDUCIAL_REG2_mid,  electron_PIDs.DC_FIDUCIAL_REG2_loose,
                                electron_PIDs.DC_FIDUCIAL_REG1_norm,  electron_PIDs.DC_FIDUCIAL_REG1_tight,  electron_PIDs.DC_FIDUCIAL_REG1_mid,  electron_PIDs.DC_FIDUCIAL_REG1_loose,
                                electron_PIDs.EC_FIDUCIAL_norm,       electron_PIDs.EC_FIDUCIAL_tight,       electron_PIDs.EC_FIDUCIAL_mid,       electron_PIDs.EC_FIDUCIAL_loose,
                                electron_PIDs.EC_SAMPLING_norm,       electron_PIDs.EC_SAMPLING_tight,       electron_PIDs.EC_SAMPLING_mid,       electron_PIDs.EC_SAMPLING_loose,
                                electron_PIDs.EC_OUTER_VS_INNER_norm, electron_PIDs.EC_OUTER_VS_INNER_tight, electron_PIDs.EC_OUTER_VS_INNER_mid, electron_PIDs.EC_OUTER_VS_INNER_loose,
                                electron_PIDs.CC_NPHE_norm,           electron_PIDs.PID_norm,
                                
                                // Pi+ Pion PID Cuts
                                pip_pion_PIDs.Full_norm,              pip_pion_PIDs.Full_tight,              pip_pion_PIDs.Full_mid,              pip_pion_PIDs.Full_loose,
                                pip_pion_PIDs.DELTA_VZ_norm,          pip_pion_PIDs.DELTA_VZ_tight,          pip_pion_PIDs.DELTA_VZ_mid,          pip_pion_PIDs.DELTA_VZ_loose,
                                pip_pion_PIDs.DC_FIDUCIAL_REG_norm,   pip_pion_PIDs.DC_FIDUCIAL_REG_tight,   pip_pion_PIDs.DC_FIDUCIAL_REG_mid,   pip_pion_PIDs.DC_FIDUCIAL_REG_loose,
                                pip_pion_PIDs.DC_FIDUCIAL_REG3_norm,  pip_pion_PIDs.DC_FIDUCIAL_REG3_tight,  pip_pion_PIDs.DC_FIDUCIAL_REG3_mid,  pip_pion_PIDs.DC_FIDUCIAL_REG3_loose,
                                pip_pion_PIDs.DC_FIDUCIAL_REG2_norm,  pip_pion_PIDs.DC_FIDUCIAL_REG2_tight,  pip_pion_PIDs.DC_FIDUCIAL_REG2_mid,  pip_pion_PIDs.DC_FIDUCIAL_REG2_loose,
                                pip_pion_PIDs.DC_FIDUCIAL_REG1_norm,  pip_pion_PIDs.DC_FIDUCIAL_REG1_tight,  pip_pion_PIDs.DC_FIDUCIAL_REG1_mid,  pip_pion_PIDs.DC_FIDUCIAL_REG1_loose,
                                pip_pion_PIDs.CHI2PID_CUT_norm,       pip_pion_PIDs.CHI2PID_CUT_tight,       pip_pion_PIDs.CHI2PID_CUT_mid,       pip_pion_PIDs.CHI2PID_CUT_loose,
                                pip_pion_PIDs.FORWARD_norm,           pip_pion_PIDs.PID_norm,
